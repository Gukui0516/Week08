...
public class FruitMergeController : MonoBehaviour
{
    ...
    private void Awake()
    {
        ...
        if (fruitData == null)
        {
            ...
            LogSystem.PushLog(LogLevel.ERROR, "MissingComponent",
                $"[FruitMergeController] {gameObject.name}에 FruitMergeData 컴포넌트가 없습니다!", useUnityDebug: true);
            enabled = false;
        }
    }
    ...
    private IEnumerator MergeCoroutine(FruitMergeController other, Vector3 contactPoint)
    {
        ...
        // [추가] b. 병합 이벤트 로그 (카운터 호출 포함)
        if (WatermelonGameManager.Instance != null)
        {
            // 병합된 *결과* 과일 타입 (예: 포도+포도 -> 사과가 되었으므로 '사과' 타입)
            var nextType = fruitData.NextFruitType;

            // 카운터 증가 및 값 받아오기
            var (totalCount, typeCount) = WatermelonGameManager.Instance.IncrementMergeCount(nextType);
    ...
    private bool ValidateOther(FruitMergeController other)
    {
        ...
        // [변경] LogLevel.WARNING
        LogSystem.PushLog(LogLevel.WARNING, "MergeTargetLost",
            "[FruitMergeController] 상대 과일이 이미 파괴됨. 병합 중단.", useUnityDebug: true);
        return false;
    }
    ...
    private void SpawnNextFruit(Vector3 position)
    {
        ...
        if (WatermelonObjectPool.Instance == null)
        {
            ...
            LogSystem.PushLog(LogLevel.ERROR, "MissingManager",
                "[FruitMergeController] WatermelonObjectPool이 없습니다!", useUnityDebug: true);
            return;
        }
        ...
        GameObject newFruit = WatermelonObjectPool.Instance.GetFruit( // [오타 수정] WatermelonObjectPool
    ...
    {
        if (WatermelonObjectPool.Instance == null) // [오타 수정] WatermelonObjectPool
        {
            // 풀이 없으면 파괴
            Destroy(gameObject);
            if (other != null && other.gameObject != null)
        ...
        // 원래 타입으로 반환
        WatermelonObjectPool.Instance.ReturnFruitByOriginalType(gameObject, type1); // [오타 수정] WatermelonObjectPool

        if (other != null && other.gameObject != null)
            WatermelonObjectPool.Instance.ReturnFruitByOriginalType(other.gameObject, type2); // [오타 수정] WatermelonObjectPool
