...
public class SequentialBombConverter : MonoBehaviour
{
    ...
    private bool isProcessingBombChange = false;
    ...
    private void Initialize()
    {
        ...
        // ✅ 여기서 이벤트 구독 (BombManager가 확실히 초기화된 후)
        if (BombManager.Instance != null)
        {
            BombManager.Instance.OnBombCountChanged += OnBombCountChanged;
            ...
            if (enableDebugLog)
            {
                Debug.Log("[SequentialBombConverter] BombManager 이벤트 구독 성공");
            }
        }
        else
        {
            Debug.LogError("[SequentialBombConverter] BombManager가 없어서 이벤트 구독 실패!");
            enabled = false;
            return;
        }
    ...
    private void OnBombCountChanged(int currentActiveBombs)
    {
        ...
        // ✅ 재진입 방지
        if (isProcessingBombChange) return;
        ...
        if (currentActiveBombs < previousActiveBombs)
        {
            ...
            isProcessingBombChange = true;  // ✅ 플래그 ON

            ConvertNextBlockToBomb();
            UpdatePreviousBombCount();  // ← 당신이 이미 만든 메서드

            isProcessingBombChange = false;  // ✅ 플래그 OFF
            return;
        }
    ...
    /// <summary>
    /// previousActiveBombs를 업데이트
    /// </summary>
    private void UpdatePreviousBombCount()
    {
